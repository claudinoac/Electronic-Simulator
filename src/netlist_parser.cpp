...
void netlist_parser::showNodes(FILE *stdout)                              //Função para imprimir a lista de nós em um arquivo externo. Recebe um ponteiro pro arquivo de saída
{
...
void netlist_parser::showCompInfo(FILE *stdout)                           //Função para imprimir a lista de componentes com informações sobre os mesmos
{
  ...
  for(i=0;i<numComp;i++)                                  //Loop para imprimir dados de cada componente, realizando uma varredura no array de struct
  {
    ...
    switch(componentes[i].type)                           //Classifica os componentes e imprime as informações de acordo com o tipo
    {
      case 'R':                                                 //Resistor
      {
        ...
        fprintf(stdout,"n+[%d]\t\t",componentes[i].n1);
        fprintf(stdout,"n-[%d]\t\t;\t",componentes[i].n2);
        fprintf(stdout,"Value=%g\t\t;\t",componentes[i].value);
        fprintf(stdout,"G2=%s\n",btoa(componentes[i].G2));
        break;
      }
      case 'L':                                                 //Indutor
      {
      ...
      case 'E':                                                //Fonte de tensão dependente de tensão
      {
        ...
        fprintf(stdout,"n+[%d]\t",componentes[i].n1);
        fprintf(stdout,"n-[%d]\t",componentes[i].n2);
        fprintf(stdout,"nC+[%d]\t",componentes[i].n3);
        fprintf(stdout,"nC-[%d]\t;\t",componentes[i].n4);
        fprintf(stdout,"Value=%g\n",componentes[i].value);
        fprintf(stdout,"G2=%s\n",btoa(componentes[i].G2));
        break;
      }
      case 'F':                                                //Fonte de corrente controlada por corrente
      {
        ...
        fprintf(stdout,"n+[%d]\t",componentes[i].n1);
        fprintf(stdout,"n-[%d]\t",componentes[i].n2);
        fprintf(stdout,"Ctrl: %s\t",componentes[componentes[i].control].name);
        fprintf(stdout,"Value=%g\n",componentes[i].value);
        fprintf(stdout,"G2=%s\n",btoa(componentes[i].G2));
        break;
      }
      ...
      case 'Q':                                                //Transistor de Junção Bipolar
      {
        ...
        fprintf(stdout,"nC[%d]\t\t",componentes[i].n1);
        fprintf(stdout,"nB[%d]\t",componentes[i].n2);
        fprintf(stdout,"nE[%d]\t;\t",componentes[i].n3);
        fprintf(stdout,"Equation=%s\n",componentes[i].equation);
        fprintf(stdout,"G2=%s\n",btoa(componentes[i].G2));
        break;
      }
      case 'M':                                               //Transistor MOSFET
      {
        ...
        fprintf(stdout,"nG[%d]\t",componentes[i].n2);
        fprintf(stdout,"nD[%d]\t\t",componentes[i].n1);
        fprintf(stdout,"nS[%d]\t",componentes[i].n3);
        fprintf(stdout,"nSS[%d]\t;\t",componentes[i].n4);
        fprintf(stdout,"Equation=%s\n",componentes[i].equation);
        fprintf(stdout,"G2=%s\n",btoa(componentes[i].G2));
        break;
      }
...
void netlist_parser::parseValues(char *number, int cmpNum, char* cmpName)  //Função para traduzir os valores indiferente da notação e armazenar no componente pelo seu índice
{
...
int netlist_parser::nodeVDD(char *nodename) //Verifica se o nó já existe, adiciona na lista se ainda não existir e retorna o número do nó
{
...
void netlist_parser::nodeSet(int compnumber, int nodetype, char *nodename) //Associa os componentes aos seus nós respectivos
{
  switch(nodetype)
  {
    case 1: //nó positivo do componente (Coletor para TJB e Dreno para MOSFET)
    {
      componentes[compnumber].n1=nodeVDD(nodename); //Passa o número do nó para o componente
      break;
    }

    case 2: //nó negativo do componente (Base para TJB e Gate para MOSFET)
    {
      componentes[compnumber].n2=nodeVDD(nodename);
      break;
    }

    case 3: //nó positivo de saída para fontes controladas, Emissor para TJB e Source para MOSFET
    {
      componentes[compnumber].n3=nodeVDD(nodename);
      break;
    }

    case 4: //nó negativo de saída para fontes controladas
    {
      componentes[compnumber].n4=nodeVDD(nodename);
      break;
    }
  }
}
...
bool netlist_parser::begin()          //Função principal
{
  ...
  while(!feof(spicearq))                          //Loop de leitura e associação até o final do arquivo de entrada
  {
    ...
    else if(fstletter=='R' || fstletter=='L' || fstletter=='C'||fstletter=='I' || fstletter=='D')
    //Testa se a linha é um componente de dois pinos
    {
      componentes[numComp].n3=-1; //Seta os pinos 3 e 4 como não existentes
      componentes[numComp].n4=-1;
    ...
    else if(fstletter=='V')
    {
      ...
      componentes[numComp].type=toupper(fstletter);
      compFlag=true;
      componentes[numComp].G2=true;
    }
    else if(fstletter=='Q'|| fstletter=='M')  //Verifica se o componente é um transistor
    {
      componentes[numComp].n3=0;
      componentes[numComp].n4=-1;
      componentes[numComp].type=toupper(fstletter);
      compFlag=true;               //Define a linha como componente
    }
    else if( fstletter=='E'|| fstletter=='H')  //Verifica se o componente é uma fonte controlada
    {
      componentes[numComp].n3=0;
      componentes[numComp].n4=0;
      componentes[numComp].type=toupper(fstletter);
      compFlag=true;               //Define a linha como componente
      componentes[numComp].G2=true;
    }
    else if(fstletter=='F' || fstletter=='G')
    {
      componentes[numComp].n3=-1;
      componentes[numComp].n4=-1;
      componentes[numComp].type=toupper(fstletter);
      compFlag=true;               //Define a linha como componente
      ...
      while(line[strcount]!='\n') //Separa a linha em palavras até achar o final da linha
      {
        ...
        switch(wordcount)       //Associa o componente com suas informações durante a varredura da linha que foi quebrada em palavras
        {
          ...
          case 3:                                                 //Associa o terceiro nó, mas antes faz uma verificação do componente
          {
            ...
            else if(componentes[numComp].type=='E' || componentes[numComp].type=='H')
            componentes[numComp].control=sscanf(words[wordcount],"%d");
            else
          ...
          case 4:
          {
            if(componentes[numComp].n4==0)                     //Verifica se é uma fonte controlada
  ...
  {
    if(componentes[i].G2 && (componentes[i].type=='E' || componentes[i].type=='F' ||
      componentes[i].type=='G'||  componentes[i].type=='H'))
    {
      for(int j=0;j<numComp;j++)
      {
        if(componentes[i].type=='E' || componentes[i].type=='H')
        {
          if(componentes[i].n3==componentes[j].n1 || componentes[i].n3==componentes[j].n2
          || componentes[i].n4==componentes[j].n1 || componentes[i].n4==componentes[j].n2)
          {
            componentes[j].G2=true;
            componentes[i].control=j;
          }
        }
        else if(componentes[i].type=='F' || componentes[i].type=='G')
          componentes[componentes[i].control].G2==true;
      }
